...
rlSocket       sock("192.168.0.55",502,1);
rlSharedMemory shm("automation/shm/modbus2.shm",206);
rlMailbox      mbx("automation/mbx/modbus2.mb");
rlThread       thread;
rlThread       watchdog;
short          lifeCounter     = 0;
...
void *reader(void *arg)
{
  ...
  while((buflen = mbx.read(buf,sizeof(buf))) > 0)
  {
    ...
    if(ret < 0)
    {
      ...
      shm.write(PLC2_WRITE_ERROR_COUNT_BASE,&writeErrorCount,2);
    }
...
int modbusCycle(int offset, int slave, int function, int start_adr, int num_register)
{
  ...
  else
  {
    ...
    shm.write(PLC2_READ_ERROR_COUNT_BASE,&readErrorCount,2);
  }
...
int main(int ac, char **av)
{
  ...
  shm.write(PLC2_LIFE_COUNTER_BASE,&lifeCounter,2);
  shm.write(PLC2_READ_ERROR_COUNT_BASE,&readErrorCount,2);
  shm.write(PLC2_WRITE_ERROR_COUNT_BASE,&writeErrorCount,2);
  while(1)
  {
    ...
    ret = modbusCycle(offset,1,3,100,50);
    if(ret>0) offset += ret; else continue;
    ret = modbusCycle(offset,1,3,150,50);
    if(ret>0) offset += ret; else continue;
  }
